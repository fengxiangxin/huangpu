<template>
    <div class="tree">
        <el-tree :data="data" show-checkbox node-key="id" @node-click="handleNodeClick" @check-change="handleCheckChange" :default-expand-all="true" :render-after-expand="false" :check-on-click-node="true" :props="defaultProps">
        </el-tree>
    </div>
</template>
            ...
            switch (data.id) {
                case '2':
                    if (isCheck) {
                        ...
                        const poiArr = []
                        const temp0 = yangan.data.data.filter((item, index) => {
                            if (index < 200) {
                                return true
                            }
                        })
                        ...
                        //         const coord = [res.coordinates[0][0] + 40000, res.coordinates[0][1] - 2330000, 20]
                ...
                case '3':
                    if (isCheck) {
                        ...
                        const poiArr = []
                        const temp0 = lajitong.data.data.filter((item, index) => {
                            if (index < 200) {
                                return true
                            }
                        })
                ...
                case '4':
                    if (isCheck) {
                        ...
                        const poiArr = []
                        const temp0 = jiucun.data.data.filter((item, index) => {
                            if (index < 200) {
                                return true
                            }
                        })
