<template>
    <div id="app">
        <div class="back">
            <img src="https://up.23magic.com/image/20190816/1565924177727505.jpg" width="6400px" height="2400px" />
        </div>
        <div id="player"></div>
        <div ref="zoom">
            <Header></Header>
...
export default {
    ...
    created() {
        ...
        this.tagdata = [...shop.pois, ...shequ.pois, ...school.pois, ...yiyuan.pois, ...yule.pois]
        ...
        async onEvent(e) {
            ...
            if (e.Type === 'tag') {
                ...
                if (e.Id.slice(0, 4) === 'tag1') {
                    ID = e.Id.split('+')[1]
                    data = require('./assets/json/yanganqi.json').data.data.filter((item, index) => {
                ...
                if (e.Id.slice(0, 4) === 'tag2') {
                    ID = e.Id.split('+')[1]
                    data = require('./assets/json/lajitong.json').data.data.filter((item, index) => {
                        if (index < 200) return true
                    })
                ...
                if (e.Id.slice(0, 4) === 'tag3') {
                    ID = e.Id.split('+')[1]
                    data = require('./assets/json/jiucun.json').data.data.filter((item, index) => {
                        if (index < 200) return true
                        // }
                    })
                }
            ...
            if (e.Type == 'shapefilelayer' && e.Fields.建筑物高度 && this.goBuildingAlone) {
